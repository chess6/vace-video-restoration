...
candidate reading reports/pilot_results.md.
...
Metrics, and what each is evidence for:
  ...
  sharpness_balance      subject / background. Near 1.0 means the subject and the
  ...
  hf_invention           high-frequency energy added where the source had none.
                         ...
                         mutated distant anchors: those all show up as new detail
...
Attribute fidelity is measured against the SOURCE interval, never against the
external references - they show a different appearance, so agreeing with them
would be the failure, not the goal. The brief pulls in two directions and the
metrics are kept apart accordingly:

Read them in that order: attribute CLASS and coverage first, then boundaries,
...
would only match a missing attribute to the palette of the one it replaced.

  attribute_deltaE         area-weighted colour distance from the source, measured
  ...
  attribute_temporal_dE    how much that colour crawls between frames
  attribute_iou            silhouette overlap with the source's parsed attribute
  attribute_boundary_f     edge agreement at 2 px, which IoU is too blunt to see
  attribute_lowfreq_drift  blurred difference inside the attribute. Must stay SMALL:
                         this is the structure that was supposed to survive.
  attribute_hf_gain        high-frequency energy vs the source. Should be ABOVE 1
  ...
  attribute_pattern_chi2   gradient-orientation histogram distance - a changed
...
def attribute_metrics(path: Path, mask_path: Path, source_palette: dict,
                    n_probe: int, log) -> dict:
    """Did the attributes keep the colour the SOURCE says they are?

    Parses attributes on the variant itself and compares each attribute class with
    the palette learned from the original footage. Two separate failures are
    distinguished, because they have different causes:

      attribute_deltaE      distance from the source colour. Large = the model
                          repainted the attributes, which is the fault reported on
                          the earlier build.
      attribute_temporal_dE spread of that colour across frames. Large = the
    ...
    for name, ref in source_palette.items():
        ...
        seen.append(name)
    if not drift:
        return {"attribute_classes_found": 0}
    return {"attribute_classes_found": len(seen),
            "attribute_classes": seen,
            ...
            # which is why the primary attribute_deltaE is measured same-frame in
            # attribute_structure() instead. Kept only as a cross-check.
            "attribute_deltaE_vs_pack_palette": round(float(np.mean(drift)), 2),
            "attribute_deltaE_vs_pack_palette_worst": round(float(np.max(drift)), 2),
            ...
            "attribute_temporal_dE": round(float(np.mean(temporal)), 2)}
...
def _boundary_f(a: np.ndarray, b: np.ndarray, tol: int = 2) -> float:
    ...
    a few pixels, which is precisely the failure that reads as a redrawn attribute.
...
def attribute_structure(var_path: Path, src_path: Path, mask_path: Path,
                      n_probe: int, log) -> dict:
    """Everything about the attribute that colour alone cannot catch.
    ...
    separately rather than fused into one score.
      ...
      attribute_iou           overlap of the parsed attribute region with the
                            source's. Low = the model redrew the silhouette.
      attribute_boundary_f    edge agreement at 2 px. Catches an edge that slid
                            while the region stayed roughly the same size.
      attribute_lowfreq_drift mean |variant - source| inside the attribute after
                            heavy blur. This must stay SMALL: it is the
                            structure and colour that were supposed to survive.
      attribute_hf_gain       high-frequency energy vs the source inside the
                            attribute. This should be ABOVE 1: it is the texture
                            the restoration exists to add. Below 1 means the
                            pass smoothed the attributes instead of restoring them.
      attribute_pattern_chi2  distance between gradient-orientation histograms.
      ...
      accessories_*         accessory classes invented or dropped, by name. An
                            invented accessory is not a texture detail.
    """
    import cv2
    from PIL import Image
    from make_reference_pack import LBL, ATTRIBUTE, Parser

    pairs = _probe_pairs(var_path, src_path, mask_path, n_probe)
    if not pairs:
        return {}
    parser = Parser(log)
    gids = [i for i, n in LBL.items() if n in ATTRIBUTE]
    ...
    for _, s_rgb, v_rgb in pairs:
        ...
        for idx, name in LBL.items():
            ...
            if name in ACCESSORY:
                ...
                # the parser is not an invented accessory.
                sa, sb = a.mean(), b.mean()
        ...
        # Attribute colour drift, measured on THIS frame over regions the SOURCE's
        ...
        # present in all six, and SegFormer moves the same physical attribute
        # between `dress`, `upper` and `skirt` from frame to frame, so per-class
        # comparison was not comparing the same attribute.
    ...
    out = {
        "attribute_frames_probed": len(ious),
        "attribute_iou": round(float(np.median(ious)), 4),
        "attribute_boundary_f": (round(float(np.median(valid_bf)), 4)
                               if valid_bf else None),
        "attribute_lowfreq_drift": round(float(np.median(lows)), 2),
        "attribute_hf_gain": round(float(np.median(hfs)), 3),
        "attribute_pattern_chi2": round(float(np.median(chis)), 4),
        "attribute_class_iou": {k: round(float(np.median(v)), 4)
        ...
        "attribute_deltaE": (round(float(np.median(drift_w)), 2)
                           if drift_w else None),
        "attribute_deltaE_worst_class": (round(float(np.median(drift_worst)), 2)
                                       if drift_worst else None),
        "attribute_deltaE_by_class": {
            ...
            # seen once is a parser flicker, not an attribute.
            if len(v) >= max(2, len(ious) // 2)},
        "appearance_mutation": bool(drift_w and float(np.median(drift_w)) > 10.0),
    }
    # The two directions of the brief, stated as one boolean each so a report
    # cannot quietly average them into "fine".
    out["structure_preserved"] = bool(out["attribute_iou"] > 0.75
                                      and out["attribute_lowfreq_drift"] < 8.0)
    # Colour is the LAST question, and only a meaningful one while the attribute
    # is still there. Once the silhouette has moved this far, a colour offset is
    # not the defect and correcting it would only make a missing attribute better
    # matched to the palette of the one it replaced.
    out["colour_is_meaningful"] = bool(out["attribute_iou"] > 0.60
                                       and out["attribute_lowfreq_drift"] < 12.0)
    out["chroma_correction_advised"] = bool(
        out["colour_is_meaningful"] and (out.get("attribute_deltaE") or 0) > 2.3)
    ...
    # reach. Pattern agreement (attribute_pattern_chi2) and temporal stability are
    # reported alongside it; a restoration claim needs all three to hold.
    out["high_frequency_increased"] = bool(out["attribute_hf_gain"] > 1.0)
    return out


def reference_match(log):
    """Anchor embeddings of the consensus-verified references, once per run.

    Delegates to match.resolve_targets so that "the candidate the references
    agree on" means exactly the same thing here, in the pack and in tracking.

    It previously walked inputs/references itself, took the LARGEST anchor in each
    reference via anchor_detail(), and never consulted the exclusion list. Both
    are the failure modes reference_match.py exists to prevent: the largest anchor in a
    reference is not necessarily the target, and a reference the user excluded
    ...
    bank = res.get("bank")
    if bank is None:
        log.warning("No verified reference match; match metrics skipped")
    return models, bank


def covering_metrics(var_path: Path, src_path: Path, mask_path: Path,
                     n_probe: int, log) -> dict:
    """Did the pass remove an anchor covering, or invent an uncovered anchor?

    A covering is an attribute. Removing it is not an improvement that scored
    badly on some axis - it is the model giving the candidate a different
    attribute, and no colour or sharpness metric notices it at all. So it is asked
    directly: compare the anchor region's covered and exposed composition against
    the source.
    """
    import cv2
    from PIL import Image
    from make_reference_pack import ANCHOR_REGION, COVERING, LBL, Parser
    ...
    anchor_ids = [ids[n] for n in ANCHOR_REGION]
    anchor_id = ids["face"]

    s_cov, v_cov, s_anchor, v_anchor = [], [], [], []
    for _, s_rgb, v_rgb in pairs:
        ls = parser.parse(Image.fromarray(s_rgb))
        lv = parser.parse(Image.fromarray(v_rgb))
        hs, hv = np.isin(ls, anchor_ids), np.isin(lv, anchor_ids)
        if not hs.any():
            continue
        ...
        s_anchor.append(float((ls == anchor_id).sum()) / max(1, int(hs.sum())))
        v_anchor.append(float((lv == anchor_id).sum()) / max(1, int(hv.sum()) or 1))
    if not s_cov:
        return {}
    sc, vc = float(np.median(s_cov)), float(np.median(v_cov))
    sf, vf = float(np.median(s_anchor)), float(np.median(v_anchor))
    covered = sc > sf or sf < 0.15
    # Removed: the source's anchor region was substantially covered and the
    # variant's is not, with exposed anchor appearing where there was none.
    removed = bool(covered and vc < 0.5 * sc and vf > sf + 0.10)
    return {"source_covering_fraction": round(sc, 4),
            "variant_covering_fraction": round(vc, 4),
            "source_exposed_fraction": round(sf, 4),
            "variant_exposed_fraction": round(vf, 4),
            "source_anchor_covered": bool(covered),
            "covering_removed": removed,
            "covering_retention": round(vc / sc, 3) if sc > 1e-6 else None}


def match_metrics(path: Path, mask_path: Path, models, bank, n_probe: int,
                     log) -> dict:
    """How much the generated anchor agrees with the external references.

    This is the ONLY thing the external references are allowed to influence, so
    it is measured and reported entirely apart from attribute fidelity. Comparing
    the same number for the source says whether conditioning improved match
    or merely changed it - a variant that agrees less than the 240p source did
    has made the anchor worse, however sharp it looks.
    """
    import cv2
    from PIL import Image
    ...
    for i in idx:
        ...
        emb, _, _ = anchor_detail(models, Image.fromarray(rgb))
        if emb is None:
            continue
        found += 1
        sims.append(float(np.max(bank @ emb)))
    if not sims:
        return {"match_anchor_frames": 0,
                "match_note": "no anchor was detectable in the subject region"}
    return {"match_anchor_frames": found,
            "match_frames_probed": int(len(idx)),
            "match_similarity": round(float(np.median(sims)), 4),
            "match_similarity_worst": round(float(np.min(sims)), 4)}
...
def main() -> int:
    ...
    ap.add_argument("--no-attribute", action="store_true",
                    help="Skip attributes parsing (faster; loses colour drift)")
    ap.add_argument("--attribute-frames", type=int, default=6)
    ap.add_argument("--no-match", action="store_true",
                    help="Skip anchor match scoring (faster; loses the "
                         "separate match-from-references measurement)")
    ...
    # The palette the SOURCE says this appearance is. Everything is measured against
    # this, not against the external references, which showed a different one.
    source_palette = {}
    ...
    if pack_dir.exists():
        ...
        if packs:
            ...
            log.info("Attribute reference: %d class(es) from the source - %s",
    ...
    if not args.no_match:
        try:
            id_models, id_bank = reference_match(log)
        except Exception as e:
            log.warning("Match scoring unavailable (%s); attribute and "
    ...
    for name, entry in variants.items():
        ...
        if source_palette and not args.no_attribute and mask_path.exists():
            try:
                m.update(attribute_metrics(path, mask_path, source_palette,
                                         args.attribute_frames, log))
            except Exception as e:
                log.warning("%s: attribute colour metrics unavailable (%s)", name, e)
        if not args.no_attribute and mask_path.exists():
            try:
                m.update(attribute_structure(path, P.root / spec["source"],
                                           mask_path, args.attribute_frames, log))
            except Exception as e:
                log.warning("%s: attribute structure metrics unavailable (%s)",
                            name, e)
        if not args.no_attribute and mask_path.exists():
            try:
        ...
        if id_bank is not None and mask_path.exists():
            try:
                im_ = match_metrics(path, mask_path, id_models, id_bank,
                                       args.attribute_frames, log)
                # If the SOURCE anchor is covered, agreement with an uncovered
                # external reference is not a score to maximise - a high value
                # would mean the covering came off. It is reported as
                # unobservable, and the covering check is what matters instead.
                if m.get("source_anchor_covered"):
                    im_ = {"match_similarity_unobservable": True,
                           "match_note": "the source anchor is covered; "
                                            "external-reference agreement cannot "
                                            "be observed and must not be "
                                            "maximised - see covering_removed",
                           "match_similarity_if_measured":
                               im_.get("match_similarity")}
                m.update(im_)
            except Exception as e:
                log.warning("%s: match metrics unavailable (%s)", name, e)
        m["path"] = rel(path)
        m["describes"] = entry.get("describes", "")
        out[name] = m
        gm = (f"  attributedE={m['attribute_deltaE']:.1f}"
              f"{'/MUTATED' if m.get('appearance_mutation') else ''}"
              if m.get("attribute_deltaE") is not None else "")
        ...
        if m.get("covering_removed"):
            log.warning("%-28s ANCHOR COVERING REMOVED: the source's anchor "
                        "region is %.0f%% covering and this variant is %.0f%%, "
                        "with the exposed fraction rising %.0f%%->%.0f%%. The "
                        "model took off an attribute the source carries.",
                        "", 100 * m["source_covering_fraction"],
                        100 * m["variant_covering_fraction"],
                        100 * m["source_exposed_fraction"],
                        100 * m["variant_exposed_fraction"])
        elif m.get("source_anchor_covered"):
            log.info("%-28s covering retained (%.2f of the source's); match "
                     "agreement is UNOBSERVABLE here and is not scored", "",
                     m.get("covering_retention") or 0.0)
        if m.get("match_similarity") is not None:
            log.info("%-28s match: agreement with the external references "
                     "%.3f (worst %.3f) on %d/%d probed frame(s)", "",
                     m["match_similarity"], m["match_similarity_worst"],
                     m["match_anchor_frames"], m["match_frames_probed"])
        elif m.get("match_anchor_frames") == 0:
            log.info("%-28s match: no anchor detectable in the subject region",
                     "")
        if m.get("attribute_iou") is not None:
            log.info("%-28s attribute: iou=%.3f boundaryF=%s lowfreq=%.1f "
                     "hf_gain=%.2f pattern=%.3f  -> structure %s, %s%s",
                     "", m["attribute_iou"],
                     ("%.3f" % m["attribute_boundary_f"]
                      if m.get("attribute_boundary_f") is not None else "n/a"),
                     m["attribute_lowfreq_drift"], m["attribute_hf_gain"],
                     m["attribute_pattern_chi2"],
                     ...
                     + ("" if m.get("colour_is_meaningful", True) else
                        "  [colour not meaningful: the attribute structure has "
