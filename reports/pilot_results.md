...
> numeric rows are measured and reproducible. A judgement the user made by
> looking is attributed to them.
...
| Pilot segment | the pilot interval, one shot, 1 x 81-frame chunk |
...
reads as "between 480p and 720p", plausible surface detail, **the finest anchor
...
differs. Anchor submask = the 4.42% VACE regenerates.

| variant | gradient in the submask | match (cosine to the bank) |
|---|---|---|
| **plate alone (no VACE)** | **1.618** | **0.2071** |
| source, unrestored | - | 0.1972 |
| VACE, no reference | 1.311 | 0.1897 |
| VACE, whole-image sheet | 1.239 | 0.1717 |
| VACE, match-only pack | 1.263 | 0.1742 |

**The plate alone wins on both metrics.** Every VACE variant is softer than what
it replaces and scores *lower* on match than the untouched source.

Caveat: `reference_match.py` treats 0.35 as the threshold for a plausible match. Every
value here is far below it, so no variant resolves match at all - these are
...
Mean absolute difference inside the submask, out of 255:

| pair | difference |
|---|---|
| no-reference vs match pack | 6.61 |
| no-reference vs whole-image sheet | 6.58 |
| **whole-image sheet vs match pack** | **1.89** |

Removing the reference entirely moves the anchor by 6.6/255. Exchanging two
...
### Regenerating the attribute is wrong under any conditioning

| attribute region | gradient | vs plate |
|---|---|---|
| plate (ground truth) | 1.837 | - |
| full-subject, whole-image sheet | 2.011 | 33.11 |
| full-subject, match-only pack | 1.203 | 35.82 |

With the attributes left in the references the model invents confident, wrong
detail (user: "totally hallucinated"). With it segmented out the model has no
attribute information at all and produces mush, 35% under the plate, and lands
*further* from the plate than before. The authority split was right: the attribute
...
|---|---|---|
| Anchor match | 1 | measured 0.17-0.21 cosine, below the 0.35 threshold, worse than the source |
| Match away from the anchor | needs human | |
| Attributes accuracy | n/a on the retained path | attribute never regenerated; 95.58% comes from the plate |
...
percent of one subject. Path B's background is bit-exact and its only cost is a
three-pixel band.
...
The aggressive plate is +36.9% detail over the source, scores best on match,
invents no attribute, and costs 2.5 minutes against 10. Adding VACE at 1.3B makes
the anchor softer and less like the references, whatever it is conditioned on.

### What is genuinely untested

Only **model capacity**. Everything above is 1.3B VACE and 3B SeedVR2.

1. **SeedVR2 7B** - no training, no match risk, targets the unresolved fine
   anchor detail, and fits 48 GB (3B peaked at 12.7 GB). Do this first.
2. **VACE-14B** - now has a correctly built match pack waiting for it.
3. **A subject LoRA** - the only route that would genuinely use the external
   references, since conditioning demonstrably does not.
...
1. **The wrong reference builder was used throughout.** `prepare_references.py`
   (Phase 5) tiles whole references - environment, attributes, watermarks and
...
3. **The composite used the subject mask as alpha.** After a `--protected` run
   that takes the whole subject from VACE's VAE round trip, costing 8.0% of the
   attribute's detail for nothing. The new `--mask` flag points it at the
   protected submask instead; attribute detail returns to parity (1.850 vs 1.837).
